...
//rxjs
import { Subject, Subscription, forkJoin, takeUntil, take } from 'rxjs';
//model
...
export class EditEmpleadoComponent implements OnInit {
  ...
  setDepartamentoFormValue(idDepto: string) {
    this.departamentos.forEach(ele => {
      if (ele.idDepto == idDepto) {
        this.empleadoForm.controls['departamento'].setValue(ele.departamento)
      }
    })
  }
  ...
  validarEdicionEmpleado(formDirective: FormGroupDirective) {0
    ...
    this.mostrarModalSiNo('¿Desea guardar los datos del empleado?').subscribe(
      {next: (res)=> {
        ...
        if (res) {           
          ...
          this.guardarEmpleado(formDirective) 
        }
  ...
  guardarEmpleado(formDirective: FormGroupDirective) {
